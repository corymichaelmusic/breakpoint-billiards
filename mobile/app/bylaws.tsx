...
import { BYLAWS_SECTIONS } from '../constants/RulesText';
        ...
        const fetchStatus = async () => {
            ...
            try {
                ...
                const { data } = await supabase
                    .from('profiles')
                    .select('bylaws_agreed')
                    .eq('id', userId)
                    .single();
                if (data?.bylaws_agreed) {
                    setAgreed(true);
                }
            } catch (err) {
                console.error("Error fetching bylaws status", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleAgree = async () => {
        ...
        try {
            ...
            const { error } = await supabase
                .from('profiles')
                .update({ bylaws_agreed: true })
                .eq('id', userId);

            if (error) throw error;

            setAgreed(true);
            Alert.alert("Success", "You have successfully acknowledged the League Bylaws.");
        } catch (err: any) {
            console.error("Error agreeing to bylaws", err);
        ...
        <SafeAreaView className="flex-1 bg-background">
            ...
            <ScrollView className="flex-1 p-6" contentContainerStyle={{ paddingBottom: 60 }}>
                <View className="items-center mb-8">
                    <Ionicons name="document-text" size={48} color="#D4AF37" />
                    <Text className="text-primary text-2xl font-bold mt-4 text-center">BREAKPOINT BILLIARDS</Text>
                    <Text className="text-gray-400 text-sm tracking-widest uppercase mt-1">Official League Bylaws</Text>
                </View>
                ...
                {!loading && (
                    <View className="mt-8 mb-16 pt-8 border-t border-white/10 items-center">
                        ...
                        <Text className="text-gray-500 text-xs text-center mt-4 px-4 leading-5">By acknowledging, you confirm that you have read, understood, and agree to abide by the Breakpoint Billiards League Bylaws.</Text>
                    </View>
                )}
            </ScrollView>
        </SafeAreaView>
