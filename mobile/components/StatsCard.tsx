...
export default function StatsCard({ label, value, subValue, highlight }: StatsCardProps) {
    return (
        <View className={`flex-1 bg-surface p-4 rounded-lg border border-border items-center justify-center ${highlight ? 'border-primary' : ''}`}>
            <Text
                className="text-gray-400 text-xs uppercase mb-1 tracking-wider font-bold text-center w-full"
            ...
            {typeof value === 'string' || typeof value === 'number' ? (
                <Text
                    className={`text-2xl font-bold ${highlight ? 'text-primary' : 'text-foreground'} text-center w-full`}
                    style={{ includeFontPadding: false }}
                    numberOfLines={1}
                    adjustsFontSizeToFit
                >
