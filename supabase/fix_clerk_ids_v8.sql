-- FIX CLERK ID MISMATCH v8 (Aggressive Legacy Cleanup)
BEGIN;

-- 1) DROP ALL POLICIES (Aggressive List based on Codebase Scan)
-- Leagues
...
-- League Players (The troublemakers)
...
DROP POLICY IF EXISTS "Users can view their own memberships" ON public.league_players; -- From error
DROP POLICY IF EXISTS "Users can view own memberships" ON public.league_players;   -- From scan
DROP POLICY IF EXISTS "Operators can view league_players" ON public.league_players; -- From scan
DROP POLICY IF EXISTS "Operators can update league_players" ON public.league_players; -- From scan
DROP POLICY IF EXISTS "Operators can delete league_players" ON public.league_players; -- From scan
DROP POLICY IF EXISTS "Players can view league members" ON public.league_players; -- From scan
DROP POLICY IF EXISTS "Debug: Allow all select" ON public.league_players; -- From scan
...
DROP POLICY IF EXISTS "Enable read access for all users" ON public.games; -- From scan
DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON public.games; -- From scan
...
-- 5) RECREATE POLICIES (Consolidated Set)
