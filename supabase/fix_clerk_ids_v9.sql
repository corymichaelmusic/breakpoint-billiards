-- FIX CLERK ID MISMATCH v9 (The "Cascading" Final Fix)
BEGIN;

-- 0) PREEMPTIVE CLEANUP (View Dependencies)
-- We must drop the view first because it depends on the columns we are changing.
-- CASCADE is crucial because other functions (like get_match_payload) might depend on this view.
...
-- 6) RECREATE VIEW (with new Text Types)
...
-- 7) RESTORE OPTIMIZED FUNCTIONS (Optional but good hygiene if CASCADE dropped them)
-- If cascade dropped function 'get_match_payload' or others relying on this view, we might need to recreate them too.
-- For now, we assume standard views don't carry logic, but let's be safe.
