-- FIX CLERK ID MISMATCH v3 (Handle ALL RLS Policies)
BEGIN;
...
-- League Players Policies (The ones blocking us)
...
-- League Players (Recreating these specifically to fix the blocker)
CREATE POLICY "Players can view own memberships" ON public.league_players FOR SELECT USING (auth.uid() = player1_id); 
-- Note: Simplified auth check above. Original used current_setting. auth.uid() is cleaner for Supabase.

CREATE POLICY "Operators can view league memberships" ON public.league_players FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.leagues WHERE id = league_players.league_id AND operator_id = auth.uid())
);
