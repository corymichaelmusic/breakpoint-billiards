...
-- 3) ALTER COLUMN TYPES (The Core Fix)
...
-- 5) RECREATE POLICIES (With Safe Text Casting)

-- LEAGUES
CREATE POLICY "Leagues are viewable by everyone" ON public.leagues FOR SELECT USING (true); -- Restored
...
-- MATCHES
CREATE POLICY "Matches are viewable by everyone" ON public.matches FOR SELECT USING (true); -- Restored
...
-- LEAGUE PLAYERS
CREATE POLICY "League memberships are viewable by everyone" ON public.league_players FOR SELECT USING (true); -- Restored (Usually public)
...
-- GAMES
CREATE POLICY "Games viewable by everyone" ON public.games FOR SELECT USING (true); -- Restored
