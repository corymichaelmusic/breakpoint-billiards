-- FIX PROFILES UUID TO TEXT MIGRATION [FINAL V6 DYNAMIC NUCLEAR]
-- This script dynamically finds and drops ALL policies on relevant tables to guarantee no dependencies remain.
-- It avoids issues with policy naming mismatches.
...
-- This block iterates over postgres system tables and drops every policy found on our target tables.
DO $$
DECLARE
    r RECORD;
BEGIN
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='winner_id_8ball') THEN ALTER TABLE public.matches ALTER COLUMN winner_id_8ball TYPE text; END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='winner_id_9ball') THEN ALTER TABLE public.matches ALTER COLUMN winner_id_9ball TYPE text; END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='forfeited_by') THEN ALTER TABLE public.matches ALTER COLUMN forfeited_by TYPE text; END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='coin_flip_winner_id') THEN ALTER TABLE public.matches ALTER COLUMN coin_flip_winner_id TYPE text; END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='current_turn_id') THEN ALTER TABLE public.matches ALTER COLUMN current_turn_id TYPE text; END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='games' AND column_name='submitted_by') THEN ALTER TABLE public.games ALTER COLUMN submitted_by TYPE text; END IF;
     IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='games' AND column_name='breaker_id') THEN ALTER TABLE public.games ALTER COLUMN breaker_id TYPE text; END IF;
END $$;
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='winner_id_8ball') THEN ALTER TABLE public.matches ADD CONSTRAINT matches_winner_id_8ball_fkey FOREIGN KEY (winner_id_8ball) REFERENCES public.profiles(id); END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='winner_id_9ball') THEN ALTER TABLE public.matches ADD CONSTRAINT matches_winner_id_9ball_fkey FOREIGN KEY (winner_id_9ball) REFERENCES public.profiles(id); END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='forfeited_by') THEN ALTER TABLE public.matches ADD CONSTRAINT matches_forfeited_by_fkey FOREIGN KEY (forfeited_by) REFERENCES public.profiles(id); END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='coin_flip_winner_id') THEN ALTER TABLE public.matches ADD CONSTRAINT matches_coin_flip_winner_id_fkey FOREIGN KEY (coin_flip_winner_id) REFERENCES public.profiles(id); END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='matches' AND column_name='current_turn_id') THEN ALTER TABLE public.matches ADD CONSTRAINT matches_current_turn_id_fkey FOREIGN KEY (current_turn_id) REFERENCES public.profiles(id); END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='games' AND column_name='submitted_by') THEN ALTER TABLE public.games ADD CONSTRAINT games_submitted_by_fkey FOREIGN KEY (submitted_by) REFERENCES public.profiles(id); END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='games' AND column_name='breaker_id') THEN ALTER TABLE public.games ADD CONSTRAINT games_breaker_id_fkey FOREIGN KEY (breaker_id) REFERENCES public.profiles(id); END IF;
END $$;
